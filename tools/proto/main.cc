...
void show_help(const char *argv_0) {
  //   |---v---|---v---|---v---|---v---|---v---|---v---|---v---|---v---|---v---|---v---|
  fprintf(stderr,
          "Usage: %s <device> <type> <command> <args>..\n"
          "Commands:\n"
          "    read <addr>\n"
          "        Read a 32-bit word.\n"
          "    write <addr> <value>\n"
...
int cmd_read(const std::string &dev_name, const std::string &type,
  ...
  try {
    if (type == "dma") {
      DevDMA devdma(base, 0x1000, true, dev_name);
      printf("%08x\n", devdma.read_u32(offset));
    } else {
...
int cmd_write(const std::string &dev_name, const std::string &type,
              const std::vector<std::string> &args) {
  if (args.size() != 3) {
    fprintf(stderr, "wrong argument size\n");
    return 1;
...
int cmd_dump(const std::string &dev_name, const std::string &type,
             const std::vector<std::string> &args) {
  if (args.size() != 4) {
    fprintf(stderr, "wrong argument size\n");
    return 1;
  ...
  try {
    ...
    if (type == "dma") {
      DevDMA devdma(base, aligned_size, true, dev_name);
      devdma.read(buf, offset, size);
    } else {
...
int cmd_load(const std::string &dev_name, const std::string &type,
             const std::vector<std::string> &args) {
  if (args.size() != 4) {
    fprintf(stderr, "wrong argument size\n");
    return 1;
...
std::unordered_map<std::string, CMD_FUNC *> cmd_map = {
    {"read", cmd_read}, {"write", cmd_write}, {"dump", cmd_dump},
    {"load", cmd_load}, {"uart", cmd_uart},
};
...
int main(int argc, const char *argv[]) {
  ...
  return it->second(dev_name, type, args);
}
